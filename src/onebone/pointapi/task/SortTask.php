...
class SortTask extends AsyncTask{
    private $sender, $rebirthpointData, $addOp, $page, $ops, $banList;
    ...
    private function getTopList(){
        ...
        $n = 1;
        $this->max = ceil((count($toppoint) - count($banList) - ($this->addOp ? 0 : count($ops))) / 5);
        $this->page = (int)min($this->max, max(1, $this->page));

        foreach($toppoint as $p => $m){
            $p = strtolower($p);
